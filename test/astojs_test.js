...
describe('comments:', function () {
    it('single-line comment below', generateTest(
        "\n\"foo\"\n-- comment below!",
        // @TODO fix this to preserve the leading newline
        "\"foo\"// comment below!"));
    it('single-line comment above', generateTest(
        "-- comment above!\n\"foo\"",
        "// comment above!\n\"foo\""));
    it('single-line comment to the right', generateTest(
        '"foo" -- EOL comment',
        '"foo"// EOL comment'));

    it('block comment below', generateTest(
        "\n\"foo\"\n(* comment below!\n\nHI*)",
        "\"foo\"/* comment below!\n\nHI*/"));
    it('block comment above', generateTest(
        "(* comment above!\nHELLO*)\"foo\"",
        "/* comment above!\nHELLO*/\n\"foo\""));
    it('block comment to the right', generateTest(
        '"foo" (* EOL comment *)',
        '"foo"/* EOL comment */'));
});
...
describe('literals:', function () {
    ...
    it('single-element record', generateTest(
        '{ product: "pen" }',
        'new Record({product:"pen"})'));
    it('multi-element record',  generateTest(
...
describe('positional subroutines:', function () {
    it('simple subroutine with no parameters', generateTest(
        'on helloWorld() return a end',
        'function helloWorld(){return a}'));
    it('simple subroutine with one parameters', generateTest(
        'on helloWorld(a) return a end',
        'function helloWorld(a){return a}'));
    it('simple positional subroutine with a couple parameters', generateTest(
        "on minimumValue(x, y) x end minimumValue",
        "function minimumValue(x,y){x}"));

    it('complex positional subroutine with a couple parameters', generateTest(
        "on minimumValue(x, y)\n"+
            "if x < y then\n" +
            "   return x\n" + 
            "else\n" + 
...
describe('labelled subroutines:', function () {
    it('simple subroutine with one AS labelled parameter', generateTest(
        "on rock around the clock\n"+
            "return clock\n"+
        "end",
        'function rock(args){var clock=args["around"];return clock}'));

    it('simple subroutine with one direct parameter', generateTest(
...
describe('script objects:', function () {
    ...
    it('script object with multiple properties', generateTest(
        "script helloWorld \n"+
            "property foo : \"bar\"\n" +
            "property bam : \"baz\"\n" +
            "property bar : \"bao\"\n" +
    ...
    it('script object with a handler', generateTest(
        "script helloWorld\n" +
            "on sayHello()\n" +
            "   return 'Hello'\n" +
            "end sayHello\n" +
    ...
    it('script object with implicit run statement', generateTest(
        "script helloWorld\n" +
            "property HowManyTimes:0\n" +
            "   return 'Hello'\n" +
        "end",
